...
def normalized_scalar_mean(image_array):
    return image_array.astype("float32").mean() / 255.0


def normalize_like_original_script(patches, scalar_mean):
    patches = patches.astype("float32") / 255.0
    patches -= scalar_mean
    return patches


def predict_patches(model, patches, batch_size, scalar_mean):
    normalized = normalize_like_original_script(patches, scalar_mean)
...
def segment_image(
    ...
    scalar_mean = normalized_scalar_mean(scaled_array)

    patches, positions, output_shape, padded_shape, crop_origin = extract_patches(scaled_array, overlap)
    predictions = predict_patches(model, patches, batch_size, scalar_mean)
